...
export async function POST(req:NextRequest) {
    ...
    return NextResponse.json(
      {
        ...
        message:`Stage ${d.difficultyStage} is not supported by ${raid.name}.`,
      },
      {status:400},
    );
  }
    ...
    return NextResponse.json(
      {
        ...
        message:`${raid.name} does not support practice parties.`,
      },
      {status:400},
    );
  }
  ...
  try {
    ...
    for(const encounterId of d.encounters) {
      await client.query(
    ...
    for(const encounterId of d.practiceEncounterIds) {
      await client.query(
    ...
    await client.query(
      "INSERT INTO party_members(party_id,user_id,status) VALUES($1,$2,'ACCEPTED')",
      [id,user.id],
    );

    await client.query(
      "INSERT INTO audit_log(user_id,action,entity_type,entity_id,metadata) VALUES($1,'PARTY_CREATE','party',$2,jsonb_build_object('raid_slug',$3::text))",
      [user.id,id,raid.slug],
    );
