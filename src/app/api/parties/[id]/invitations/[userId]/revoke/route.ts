...
export async function POST(req:NextRequest,{params}:{params:Promise<{id:string;userId:string}>}) {
  ...
  const party=await query(
    "SELECT 1 FROM parties WHERE id=$1 AND leader_id=$2 AND status IN ('OPEN','FULL')",
    [id,leader.id]
  );
  if(!party.rowCount) return NextResponse.json({error:"forbidden"},{status:403});

  const r=await query(
    "DELETE FROM party_members WHERE party_id=$1 AND user_id=$2 AND status='INVITED' RETURNING user_id",
    [id,userId]
  );
  if(!r.rowCount) return NextResponse.json({error:"invitation_not_found"},{status:404});
