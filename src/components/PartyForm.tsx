...
export default function PartyForm({encounters,partySize,partyId,initial}:{encounters:E[];partySize:number;partyId?:string;initial?:Initial}){
  ...
  function togglePractice(id:string){
    setPracticeIds(v=>v.includes(id)?v.filter(x=>x!==id):[...v,id]);
  }
  ...
  async function submit(e:React.FormEvent<HTMLFormElement>){
    ...
    const body={
      ...
      isPractice:practice,
      practiceEncounterIds:practice?practiceIds:[],
  ...
  return <form onSubmit={submit} className="card form stack">
    <label>
      Optional title
      <input name="title" maxLength={80} defaultValue={initial?.title??''} placeholder="Saturday Doom full run"/>
    </label>
    ...
    <div>
      ...
      <div className="checks">
        <label>
          <input type="checkbox" checked={fullRun} onChange={()=>setSelected(fullRun?[]:allIds)}/>
          <strong>Full Run</strong>
        </label>
        {encounters.map(x=>
          <label key={x.id}>
            <input type="checkbox" checked={selected.includes(x.id)} onChange={()=>toggleEncounter(x.id)}/>
            {x.code}
          </label>
        )}
      </div>
    ...
    <div>
      ...
      <div className="choice-row">
        {[1,2,3].map(s=>
          <button type="button" key={s} className={`choice-btn ${stage===s?'selected':''}`} onClick={()=>setStage(s)}>
            Stage {s}
          </button>
        )}
      </div>
    ...
    <div className="stack">
      ...
      {practice&&
        <div>
          ...
          <div className="choice-row">
            {encounters.filter(x=>selected.includes(x.id)).map(x=>
              <button
                type="button"
                key={x.id}
                className={`choice-btn ${practiceIds.includes(x.id)?'selected':''}`}
    ...
    <div>
      ...
      <p className="muted">
        These are the roles you want for this group. You can edit them later. The total cannot exceed the raid's {partySize}-player limit.
      </p>
      ...
      <p className="muted">
        {totalRequested}/{partySize} composition slots requested. {partySize-totalRequested} slot{partySize-totalRequested===1?' is':'s are'} unrestricted.
      </p>
    </div>
    ...
    <p className="muted">
      Turn this off if your group agrees to use a different composition. You can also toggle it directly from the party page.
    </p>

    <div className="row">
      <button className="btn primary">{partyId?'Save changes':'Create party'}</button>
