...
export default function PartyCard({party,relation}:Props){
  ...
  return <article className="card party-card stack">
    <div className="party-card-head">
      <div>
        ...
        <h3 className="party-card-title">{party.title||party.encounters}</h3>
      </div>
      <div className="row">
        {relation&&<span className="pill relation-pill">{relation}</span>}
        <span className={`pill ${full?"full-pill":""}`}>{full?"FULL":"OPEN"}</span>
        <span className="pill">{party.isPractice?"Practice":"Clear"}</span>
      </div>
    </div>

    {party.creator&&<div className="muted">Created by <strong>@{party.creator}</strong></div>}
    ...
    <div className="party-summary-grid">
      ...
      <div>
        ...
        <strong><LocalDateTime iso={party.start}/></strong>
      </div>
    </div>

    <div className="party-needs">
      <span>Still wanted</span>
      <strong>{party.remainingPhysical} Physical · {party.remainingMagical} Magical · {party.remainingSupport} Support</strong>
    </div>

    <Link className="btn party-card-action" href={`/parties/${party.id}`}>
      {full?"View full party":"View party"}
    </Link>
  </article>;
}
