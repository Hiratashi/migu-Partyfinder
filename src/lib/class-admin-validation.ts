...
export const classAdminSchema=z.object({
  slug:z.string().trim().min(2).max(80).regex(
    slugPattern,
    "URL name must contain lowercase letters, numbers and hyphens only",
  ),
  name:z.string().trim().min(2).max(100),
  abbreviation:z.string().trim().min(1).max(12),
  baseCharacter:z.string().trim().min(1).max(40),
  pathNumber:z.number().int().min(1).max(4),
  damageType:z.enum(["PHYSICAL","MAGICAL","HYBRID","NONE"]),
  role:z.enum(["DPS","SUPPORT","FLEX"]),
  iconPath:z.string().trim().max(500).optional().default(""),
  active:z.boolean(),
